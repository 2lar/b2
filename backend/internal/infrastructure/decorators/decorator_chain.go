// Package decorators provides cross-cutting concerns for repositories.
...
func (dc *DecoratorChain) DecorateNodeRepository(base repository.NodeRepository) repository.NodeRepository {
	...
	if dc.config.Features.EnableCaching && dc.cache != nil {
		decorated = NewCachingNodeRepository(
			decorated,
			dc.cache,
			CachingConfig{
	...
	if dc.config.Features.EnableMetrics && dc.metrics != nil {
		decorated = NewMetricsNodeRepository(
			decorated,
			dc.metrics,
			MetricsConfig{
				ServiceName:      "brain2",
	...
	if dc.config.Features.EnableLogging {
		decorated = NewLoggingNodeRepository(
			decorated,
			dc.logger,
			LoggingConfig{
				LogRequests:   true,
...
func (dc *DecoratorChain) DecorateEdgeRepository(base repository.EdgeRepository) repository.EdgeRepository {
	...
	// Logging
	if dc.config.Features.EnableLogging {
		decorated = NewLoggingEdgeRepository(
			decorated,
			dc.logger,
			LoggingConfig{
				LogRequests:   true,
