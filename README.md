...
```
                    🧠 Brain2 - Serverless Knowledge Management System
...
├─────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐                      │
│  │   CloudFront    │    │   S3 Bucket     │    │   TypeScript    │    │   Cytoscape.js  │                      │
│  │   (Global CDN)  │    │ (Static Assets) │    │   SPA Client    │    │  (Graph Viz)    │                      │
│  │                 │    │                 │    │                 │    │                 │                      │
│  │ • HTTPS/HTTP2   │    │ • HTML/CSS/JS   │    │ • Event Driven  │    │ • Force Layout  │                      │
│  │ • Compression   │    │ • Auto Deploy   │    │ • Type Safety   │    │ • Interactive   │                      │
│  │ • Edge Caching  │    │ • SPA Support   │    │ • API Client    │    │ • Real-time     │                      │
│  └─────────────────┘    └─────────────────┘    └─────────────────┘    └─────────────────┘                      │
│         │                        │                        │                        │                           │
│         └────────────────────────┼────────────────────────┼────────────────────────┘                           │
│                                  │                        │                                                    │
...
│  │  │                                             │  │                                                 │   │ │
│  │  │ Routes: /api/{proxy+}                       │  │ Routes: $connect, $disconnect, $default         │   │ │
│  │  │ Methods: GET, POST, PUT, DELETE              │  │ Auth: JWT in query parameters                   │   │ │
│  │  │ CORS: Enabled for all origins                │  │ Management: Server-to-client messaging         │   │ │
│  │  │                                             │  │                                                 │   │ │
│  │  │ ┌─────────────────────────────────────────┐ │  │ ┌─────────────────────────────────────────────┐ │   │ │
│  │  │ │     🔐 Lambda Authorizer                │ │  │ │         Connection Management               │ │   │ │
│  │  │ │                                         │ │  │ │                                             │ │   │ │
│  │  │ │ • JWT Token Validation                  │ │  │ │ • Connection Tracking                       │ │   │ │
│  │  │ │ • Supabase Integration                  │ │  │ │ • TTL-based Cleanup                         │ │   │ │
│  │  │ │ • 5-minute Result Caching               │ │  │ │ • Real-time Message Broadcasting            │ │   │ │
│  │  │ │ • Simple Allow/Deny Response            │ │  │ │ • Automatic Reconnection Support           │ │   │ │
│  │  │ └─────────────────────────────────────────┘ │  │ └─────────────────────────────────────────────┘ │   │ │
│  │  └─────────────────────────────────────────────┘  └─────────────────────────────────────────────────┘   │ │
│  │                             │                                              │                              │ │
│  └─────────────────────────────┼──────────────────────────────────────────────┼──────────────────────────────┘ │
...
│                                │                                              │                                │
│  ┌─────────────────────────────▼─────────────────────────────┐    ┌───────────▼────────────────────────────────┐ │
│  │                    📋 HTTP LAMBDA                          │    │              📡 WEBSOCKET LAMBDAS         │ │
│  │                                                           │    │                                            │ │
│  │  ┌───────────────────────────────────────────────────────┐ │    │  ┌─────────────────────────────────────────┐ │ │
│  │  │          Main API Lambda (Go)                        │ │    │  │        WS-Connect Lambda (Go)           │ │ │
│  │  │                                                       │ │    │  │                                         │ │ │
│  │  │ • Chi Router with Middleware                          │ │    │  │ • JWT Validation                        │ │ │
│  │  │ • Clean Architecture Pattern                          │ │    │  │ • Connection Storage                    │ │ │
│  │  │ • Dependency Injection                                │ │    │  │ • TTL Configuration                     │ │ │
│  │  │ • Event Publishing                                    │ │    │  └─────────────────────────────────────────┘ │ │
│  │  │                                                       │ │    │                                            │ │
│  │  │ Endpoints:                                            │ │    │  ┌─────────────────────────────────────────┐ │ │
│  │  │ • GET/POST   /api/nodes                               │ │    │  │       WS-Disconnect Lambda (Go)        │ │ │
│  │  │ • GET/PUT/DELETE /api/nodes/{id}                      │ │    │  │                                         │ │ │
│  │  │ • POST       /api/nodes/bulk-delete                   │ │    │  │ • GSI Reverse Lookup                    │ │ │
│  │  │ • GET        /api/graph-data                          │ │    │  │ • Connection Cleanup                    │ │ │
│  │  │                                                       │ │    │  │ • Graceful Error Handling               │ │ │
│  │  │ Events Published:                                     │ │    │  └─────────────────────────────────────────┘ │ │
│  │  │ • NodeCreated → EventBridge                           │ │    │                                            │ │
│  │  └───────────────────────────────────────────────────────┘ │    │  ┌─────────────────────────────────────────┐ │ │
│  └─────────────────────────────────────────────────────────────┘    │  │     WS-Send-Message Lambda (Go)        │ │ │
│                                │                                     │  │                                         │ │ │
│                                │                                     │  │ • Fan-out Message Broadcasting          │ │ │
│  ┌─────────────────────────────▼─────────────────────────────┐       │  │ • Connection Health Management          │ │ │
│  │            🧠 EVENT PROCESSING LAMBDA                     │       │  │ • Stale Connection Cleanup              │ │ │
│  │                                                           │       │  │ • API Gateway Management API           │ │ │
│  │  ┌───────────────────────────────────────────────────────┐ │       │  └─────────────────────────────────────────┘ │ │
│  │  │         Connect-Node Lambda (Go)                     │ │       └────────────────────────────────────────────────┘ │
│  │  │                                                       │ │                                                          │
│  │  │ Triggered by: "NodeCreated" Events                   │ │                                                          │
│  │  │                                                       │ │                                                          │
│  │  │ Processing Algorithm:                                 │ │                                                          │
│  │  │ 1. Extract keywords from new memory                   │ │                                                          │
│  │  │ 2. Query existing memories (GSI)                      │ │                                                          │
│  │  │ 3. Calculate connection strength                      │ │                                                          │
│  │  │ 4. Create bidirectional edges                         │ │                                                          │
│  │  │ 5. Publish "EdgesCreated" event                       │ │                                                          │
│  │  │                                                       │ │                                                          │
│  │  │ NLP Features:                                         │ │                                                          │
│  │  │ • Stop word filtering                                 │ │                                                          │
│  │  │ • Keyword extraction                                  │ │                                                          │
...
│                                │                                                                                      │
│  ┌─────────────────────────────▼─────────────────────────────────────────────────────────────────────────────────┐   │
│  │                                      🎯 EventBridge (B2EventBus)                                              │   │
│  │                                                                                                                 │   │
│  │  Event Flow:                                                                                                    │   │
│  │                                                                                                                 │   │
│  │  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐                     │   │
│  │  │  NodeCreated    │───▶│  Event Router   │───▶│  EdgesCreated   │───▶│  WebSocket      │                     │   │
│  │  │                 │    │                 │    │                 │    │   Broadcasting  │                     │   │
│  │  │ Source:         │    │ Pattern         │    │ Source:         │    │                 │                     │   │
│  │  │ brain2.api      │    │ Matching        │    │ brain2.connect  │    │ Real-time       │                     │   │
│  │  │                 │    │                 │    │                 │    │ UI Updates      │                     │   │
│  │  │ Trigger:        │    │ Rules:          │    │ Trigger:        │    │                 │                     │   │
│  │  │ Memory Creation │    │ • NodeCreated   │    │ Edge Discovery  │    │ Graph           │                     │   │
│  │  │                 │    │ • EdgesCreated  │    │                 │    │ Refresh         │                     │   │
│  │  └─────────────────┘    └─────────────────┘    └─────────────────┘    └─────────────────┘                     │   │
│  │                                                                                                                 │   │
│  └─────────────────────────────────────────────────────────────────────────────────────────────────────────────┘   │
│                                                                                                                      │
...
│  │  │  │ Data: {content, timestamp, keywords[], title, createdAt, updatedAt}                                  │ │ │ │
│  │  │  └─────────────────────────────────────────────────────────────────────────────────────────────────────┘ │ │ │
│  │  │                                                                                                           │ │ │
│  │  │  ┌─────────────────────────────────────────────────────────────────────────────────────────────────────┐ │ │ │
│  │  │  │ MEMORY CONNECTIONS (Edges)                                                                            │ │ │ │
│  │  │  │ PK: USER#{userId}#NODE#{nodeId}                                                                       │ │ │ │
│  │  │  │ SK: EDGE#RELATES_TO#NODE#{otherNodeId}                                                                │ │ │ │
│  │  │  │ Data: {sharedKeywords[], connectionStrength, createdAt, bidirectional}                               │ │ │ │
│  │  │  └─────────────────────────────────────────────────────────────────────────────────────────────────────┘ │ │ │
│  │  │                                                                                                           │ │ │
│  │  │  ┌─────────────────────────────────────────────────────────────────────────────────────────────────────┐ │ │ │
│  │  │  │ SEARCHABLE KEYWORDS                                                                                   │ │ │ │
│  │  │  │ PK: USER#{userId}#NODE#{nodeId}                                                                       │ │ │ │
│  │  │  │ SK: KEYWORD#{keyword}                                                                                 │ │ │ │
│  │  │  │ Data: {keyword, relevance, stemmed, partOfSpeech}                                                     │ │ │ │
│  │  │  └─────────────────────────────────────────────────────────────────────────────────────────────────────┘ │ │ │
│  │  │                                                                                                           │ │ │
│  │  │  ┌─────────────────────────────────────────────────────────────────────────────────────────────────────┐ │ │ │
│  │  │  │ Global Secondary Index: KeywordIndex                                                                  │ │ │ │
│  │  │  │ GSI1PK: USER#{userId}#KEYWORD#{keyword}                                                               │ │ │ │
│  │  │  │ GSI1SK: NODE#{nodeId}                                                                                 │ │ │ │
│  │  │  │ Purpose: Fast keyword-based memory discovery                                                          │ │ │ │
│  │  │  │ Queries: Find all memories containing specific keywords                                               │ │ │ │
│  │  │  └─────────────────────────────────────────────────────────────────────────────────────────────────────┘ │ │ │
│  │  └─────────────────────────────────────────────────────────────────────────────────────────────────────────┘ │ │
│  │                                                                                                               │ │
│  │  ┌─────────────────────────────────────────────────────────────────────────────────────────────────────────┐ │ │
│  │  │                            Connections Table: B2-Connections                                            │ │ │
│  │  │                                                                                                           │ │ │
│  │  │  ┌─────────────────────────────────────────────────────────────────────────────────────────────────────┐ │ │ │
...
│  │  │  │ TTL: expireAt (automatic cleanup of stale connections)                                                │ │ │ │
│  │  │  └─────────────────────────────────────────────────────────────────────────────────────────────────────┘ │ │ │
│  │  │                                                                                                           │ │ │
│  │  │  ┌─────────────────────────────────────────────────────────────────────────────────────────────────────┐ │ │ │
│  │  │  │ GSI: connection-id-index (Reverse Lookup)                                                            │ │ │ │
│  │  │  │ GSI1PK: CONN#{connectionId}                                                                           │ │ │ │
│  │  │  │ GSI1SK: USER#{userId}                                                                                 │ │ │ │
│  │  │  │ Purpose: Find user ID when connection disconnects                                                     │ │ │ │
│  │  │  └─────────────────────────────────────────────────────────────────────────────────────────────────────┘ │ │ │
│  │  └─────────────────────────────────────────────────────────────────────────────────────────────────────────┘ │ │
│  └─────────────────────────────────────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                                                   │
...
1. **Memory Creation Flow:**
   User Input → API Gateway → JWT Authorizer → Main Lambda → DynamoDB → EventBridge → ConnectNode Lambda → 
   Connection Discovery → Edge Creation → EventBridge → WebSocket Lambda → Real-time UI Update

2. **Real-Time Communication Flow:**
   WebSocket Connect → JWT Validation → Connection Storage → Event Trigger → Message Broadcasting → 
   Connection Health Check → Client Update → Graph Refresh

3. **Authentication Flow:**
   Supabase Auth → JWT Token → API Gateway Caching → Lambda Authorizer → User Context → Secure Operations

4. **Graph Intelligence Flow:**
   New Memory → Keyword Extraction → GSI Query → Similarity Calculation → Bidirectional Edge Creation → 
   Real-time Broadcast → Interactive Visualization

🏗️ **Key Architectural Patterns:**
• **Event-Driven Architecture**: Asynchronous processing with EventBridge
• **Serverless-First**: Auto-scaling Lambda functions with pay-per-use
• **Single-Table Design**: Optimized DynamoDB for graph operations
• **Real-Time Communication**: WebSocket API with connection management
• **Clean Architecture**: Separation of concerns with dependency injection
• **Infrastructure as Code**: AWS CDK for reproducible deployments
• **Security by Design**: JWT authentication with least privilege IAM
